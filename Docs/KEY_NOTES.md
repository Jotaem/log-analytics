**1. Evaluación de la Query: Riesgos Matemáticos y Soluciones**

La query actual tiene la granularidad perfecta (`partner_id` + `fecha`) para nutrir tu dashboard, pero **SÍ presenta un riesgo enorme de calcular mal las métricas (promediar promedios y duplicar bases)** si el frontend no agrupa los datos correctamente.

Aquí están las reglas estrictas que tu capa de JavaScript (frontend) deberá aplicar para que los cálculos sean exactos:

- **Regla de Ratios (CVRs y Fail Rates):** Jamás debes sumar ni promediar campos como `partner_fail_rate`, `partner_cvr_neto` o `partner_open_time_compliance_pct`. Tu frontend debe sumar los numeradores y denominadores por separado para el grupo seleccionado y luego dividir.
  - *Ejemplo correcto para un grupo:* `SUM(rejected_orders) / SUM(total_orders)`.
- **Peligro de Duplicación de Sesiones de la Ciudad:** La query repite `city_total_sessions_raw` en cada fila de los locales de esa ciudad. Si seleccionas 10 locales de Santiago y sumas las sesiones de la ciudad, tendrás 10 veces más sesiones de las reales.
  - *Solución:* Para calcular el tráfico de la ciudad en el frontend, debes desduplicar por `ciudad` + `fecha` (ej. tomando el `MAX` o el valor único de esa dupla) antes de sumar las sesiones totales del período.
- **Métricas Dinámicas (Share y Penetración):** El "Share over group" no puede venir de SQL porque el "grupo" nace en la interacción del usuario. El JS deberá sumar las órdenes totales del grupo renderizado y dividir las órdenes de cada local sobre ese gran total al vuelo.

**2. Sugerencias UX/UI Nivel Enterprise para Google Apps Script**

Para que una WebApp en GAS no se sienta lenta ni rudimentaria, debes sacar el procesamiento del lado de Google (backend) y llevarlo al navegador (cliente):

- **Arquitectura Single Page Application (SPA):** Usa **Vue.js** (recomendado por su curva de aprendizaje y potencia) o React empaquetado dentro de GAS. El backend `.gs` solo debe tener una función: recibir los filtros, ejecutar el SQL en BigQuery y devolver un JSON puro.
- **Estado Local (Evitar re-cargas):** Cuando el usuario cambie un gráfico de barras a líneas, o mueva una métrica del eje izquierdo al derecho, **no llames al servidor**. Todo el JSON ya debe estar en la memoria del navegador. Usa una librería como **ApexCharts** o **ECharts**, que animan las transiciones fluidamente.
- **Esqueletos de Carga (Skeleton Loaders):** Dado que BigQuery + GAS puede tardar 3-5 segundos, muestra *placeholders* parpadeantes con la forma de los gráficos y la tabla mientras se procesa la data.
- **Sincronización de Tooltips:** Al comparar 4 grupos en 4 gráficos distintos, configura la librería de gráficos para que, al hacer hover en el 15 de marzo del Grupo 1, se ilumine automáticamente el 15 de marzo en los otros 3 gráficos. Esto es clave en análisis comparativo.
- **Sticky Headers y Virtualización:** La tabla de ranking inferior puede tener miles de locales. Usa *virtual scrolling* (ej. `ag-Grid` o librerías de tablas para Vue/React) con cabeceras fijas para que no colapse el DOM del navegador.

**3. Query Maestra para Appscript**
...
base_sessions AS (
  ...
  WHERE s.country_id = 2 
    AND s.partition_date >= '2026-01-01' 
...
base_logistics AS (
  ...
  INNER JOIN `peya-bi-tools-pro.il_core.fact_orders` fo 
   ...
   AND DATE(fo.registered_date) >= '2026-01-01' 
   AND DATE(fo.registered_date) <= '2026-01-31'
  WHERE l.country.country_id = 2
    AND l.created_date_local >= '2026-01-01' 
    AND l.created_date_local <= '2026-01-31'
...
base_partners AS (
  ...
  LEFT JOIN `peya-bi-tools-pro.il_core.dim_area` da 
    ON dp.address.area_id = da.area_id
  LEFT JOIN `peya-bi-tools-pro.il_core.dim_historical_partners` hp 
    ON dp.partner_id = hp.restaurant_id
  LEFT JOIN `peya-bi-tools-pro.il_core.fact_orders` fo 
    ON dp.partner_id = fo.restaurant.id 
   AND DATE(fo.registered_date) >= '2026-01-01' 
   AND DATE(fo.registered_date) <= '2026-01-31' 
   AND DATE(fo.registered_date) = DATE(hp.full_date)
  LEFT JOIN base_logistics log 
    ON dp.partner_id = log.partner_id 
   AND DATE(hp.full_date) = log.fecha
  LEFT JOIN malls m 
    ON ST_CONTAINS(m.mall_polygon, ST_GEOGPOINT(dp.address.longitude, dp.address.latitude))
  WHERE (dp.country.country_code = 'CL' OR dp.country_id = 2)
    AND hp.is_active = TRUE 
    AND DATE(hp.full_date) >= '2026-01-01' 
    AND DATE(hp.full_date) <= '2026-01-31'
  GROUP BY 
    dp.city.name,
    da.area_name,
    DATE(hp.full_date),
    dp.partner_id,
    dp.partner_name,
    dp.franchise.franchise_name,
    dp.is_logistic,
    log.city_name_log,
    log.zone_name_log,
    log.total_rejected_orders,
    m.mall_name
)
...
SELECT 
  ...
  -- Estado y Tiempos de Conexión
  p.is_active_partner,
  p.schedule_open_time AS plan_open_time_minutes,
  p.real_open_time AS real_open_time_minutes,
  SAFE_DIVIDE(p.real_open_time, p.schedule_open_time) AS partner_open_time_compliance_pct,
  
  -- Órdenes del Partner y Rechazos Logísticos
  p.total_orders,
  p.confirmed_orders,
  p.rejected_orders,
  CASE WHEN p.total_orders > 0 THEN 1 ELSE 0 END AS has_orders,
  SAFE_DIVIDE(p.rejected_orders, p.total_orders) AS partner_fail_rate,
  
  -- Impacto del Local sobre el Tráfico de la Zona
  SAFE_DIVIDE(p.total_orders, s.total_sessions) AS partner_share_city_sessions,
  
  -- Tráfico y CVRs de la Zona (Ciudad)
  ...
  SAFE_DIVIDE(s.sessions_with_orders, s.total_sessions) AS city_cvr_neto,
  SAFE_DIVIDE(s.sessions_shop_details, s.sessions_shop_list) AS city_cvr_2,
  SAFE_DIVIDE(s.sessions_with_orders, s.sessions_checkout) AS city_cvr_3

FROM base_partners p
LEFT JOIN base_sessions s 
  ON p.zone_name = s.ciudad 
 AND p.fecha = s.fecha

ORDER BY p.fecha ASC, p.ciudad ASC, p.zone_name ASC, p.partner_id ASC